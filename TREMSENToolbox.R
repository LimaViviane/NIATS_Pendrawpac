...
#   {
#     
#     install.packages(remainingPackages, repos = "http://cran.us.r-project.org");
#     
#   }
#   
...
#                      
#                      'mclust', 'plyr','tsne', 'boot', 'reshape2',
#                      
...
#' @title  Linear detrend. The function uses the function detrend available in the Package 'pracma'
#'   
#' @name detrendTremsenData
#' 
#' @param df : dataframe resulting from LoadTREMSENFile
#' 
#' @param startColRange startColRange = 2
#' 
#' @param endColRange endColRange = 39
#' 
#' @description  Linear detrend. The function uses the function detrend available in the Package 'pracma'
#'              (Practical Numerical Math Functions) for removing piecewise linear trends of the data
#' 
#' @return  Linear detrended dataframe
#' 
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' @importFrom pracma detrend
#' 
#' @examples  \dontrun{ df.detrended <- detrendTremsenData(df)}
#' 
#' @export
#'
...
#' @title  Nonlinear detrend. The function removes nonlinear trends from the data. 
#'   
#' @name nonLineardetrendTremsenData
#' 
#' @param df dataframe resulting from LoadTREMSENFile
#' 
#' @param startColRange startColRange = 2
...
#'
#' 
#' @description  The data are first smoothed (see Tukey's smoothers, in  the package stats, function smooth)
#'                and then the resulting signal is subtracted from a nonlinear trend.The nonlinear trend is
#'                estimated by Local Polynomial Regression Fitting (see function loess)in the package stats.
#'                The resulting signal is also linearly detrended.
#' 
#' @return  Linear detrended dataframe
#' 
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' @examples  \dontrun{ df.nonlineardetrended <- nonLineardetrendTremsenData(df)}
#' 
#' @export
#' 
...
#' @title  Smooth data based on the Tukey's (Running Median) Smoothing (see the package smooth in the package stats) 
#'   
#' @name smoothTremsenData
#' 
#' @param df : dataframe resulting from LoadTREMSENFile
#' 
#' @param startColRange startColRange = 2
...
#' 
#' @examples \dontrun{ df.smoothed <- smoothTremsenData(df)}
...
#' @title  Apply a rectangular window to the input signal. The input signal is multiplied by PULSE (0=LOW and 1=HIGH)
#'   
#' @name windowTremsenData
#' 
#' @param df : dataframe resulting from LoadTREMSENFile
#' 
#' @param startColRange startColRange = 2
...
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' @examples \dontrun{ df.windowed <- windowTremsenData(df)}
...
#' @title  Estimate the power spectrum of the input data
#'   
#' @name psdTremsenData
#' 
#' @param df : dataframe resulting from LoadTREMSENFile
...
#'
#' 
#' @return   power spectrum based on the 'Adaptive sine multitaper power
#'           spectral density estimation'. See the function pspectrum in the package psd
#' 
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' @examples \dontrun{ pp <- psdTremsenData(df.nonlineardetrended)}
...
#' @title  Estimate features from the input data
#' 
#' @name featExtractFromTremenDataSet
#' 
#' @param df : dataframe resulting from LoadTREMSENFile
#' 
#' @param w : length of the window, from which features are estimated
#' 
#' @param s : spot step. Windows of length w are positioned on each spot, starting from 1, 
#'           being then incremented by s, to the last spot, which should be less or equal
#'           to (total - ww), where total is the number of samples of the time-series
#'
#' @param method method="rms" 
#'  
#' @param startColRange startColRange = 2
#' 
#' @param endColRange endColRange = 39
#'    
#' @return   set of estimated features for each window
#' 
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' 
...
featExtractFromTremenDataSet <- function(df, w, s, method="rms", startColRange=2, endColRange=39) {
  ...
  slideFunct <- function(data, ww, ss) {
    ...
    for (i in 1:length(spots)) {
      ...
      # Estima a ra?z do valor m?dio quadrado
      ...
      # Estima o valor absoluto m?dio de um vetor.
      ...
      # Estima o valor m?ximo de um vetor, considera somente valores positivos da janela
      ...
      # Estima a m?dia do valor absoluto da primeira diferen?a
      ...
      # Estima a m?dia do valor absoluto da primeira diferen?a do sinal normalizado
      ...
      # Estima a m?dia do valor absoluto da segunda diferen?a 
      ...
      # Estima a m?dia do valor absoluto da segunda diferen?a do sinal normalizado
...
#' @title  Estimate statistics from a set of features
#' 
#' @name getStatisticsFromWindowedTremenDataSet
#' 
#' @param df : dataframe resulting from LoadTREMSENFile
#' 
#' @param f : statistic
...
getStatisticsFromWindowedTremenDataSet <- function(df, f = median, startColRange=2, endColRange=39) {
  ...
  # condicao em que o pulso come?a em n?vel alto
...
#' @title  resample data set
#' 
#' @name resampleTremsenData
#' 
#' @param df  dataframe resulting from LoadTREMSENFile
#' 
...
#' @title   plot the power spectrum of a data set
#' 
#' @name plotPSTremsenDataSet
#' 
#' @param pp : power spectrum, as estimated by psdTremsenData
#' 
#' @param  printplot : boolean flag (if true, then it will plot the power spectrumm, otherwise a figure handle is generated)
#' 
#' @return   figure displaying the power spectrum of each time series in the data set
#' 
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' @examples 
...
#' @title   plot time-series in the data set
#' 
#' @name plotTremsenDataset
#' 
#' @param dflist  list of dataframes to be plotted
#' 
#' @param indxs  index of signals to be plotted from the data set
#' 
#' @param printplot  boolean flag (if true, then it will plot the power spectrumm, otherwise a figure handle is generated)
#' 
#' @param droplvs = ""
...
#' @param alphavec = NULL
#' 
#' @return  figure displaying the data set
#' 
#' @author Prof. Adriano de Oliveira Andrade
#' 
#' @examples 
