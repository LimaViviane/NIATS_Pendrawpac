...
#
#
# Removes the peaks at the beginning and end of the time serie that are not part of the movement.
# 
# Description:
#       A function that removes the peaks present at the beginning and end of the time series
#       that correspond to noises of unwanted hand movement.
#     
# Input:
#       eixo: Time series from one of the axes (X, Y or Z) of the accelerometer.
#
# Output: 
#       Returns the time series without noise. 
#
# Example os use: 
#       df$X <- removeSpikes(df$X)
#       df$Y <- removeSpikes(df$Y)
#       df$Z <- removeSpikes(df$Z)
